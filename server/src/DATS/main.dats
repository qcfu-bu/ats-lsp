...
#extern fun bootstrap_set_validator(f: string -> diagnostics) : void = $extnam()
#extern fun bootstrap_connect() : void = $extnam()

fun ats_validator(uri: string) : diagnostics = 
  let val ds = diagnostics_make()
      val p1 = position_make(0, 0)
      val p2 = position_make(0, 1)
      val d = diagnostic_make(
        severity_error$make(), p1, p2, "hello", uri
      )
      val () = ds.push(d)
   in ds
  end
